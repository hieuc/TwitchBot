...
{
    /** General log path. */
    private static final String ROOT_LOG_PATH = System.getProperty("user.dir") + "/log/";
    ...
    private void connect(final String host, final int port) 
    {
        try {  
            ...
            bfh = new FileHandler(fileInit(ROOT_LOG_PATH + "buglog.txt"), true);
    ...
    public void joinChannel(final String channel)
    {
        ...
        try
        {
            ...
            fh = new FileHandler(fileInit(ROOT_LOG_PATH + "chatlogs/" + channel + ".txt"), true); 
    ...
    /**
     * Make sure the path to file exist.
     * Return a string of given path.
     *  
     * @param path to file
     * @return given path string
     */
    private String fileInit(final String path) {
        final File file = new File(path);
        try {
            if (!file.exists()) {
                // make directory
                Files.createDirectories(Paths.get(ROOT_LOG_PATH));
                // make text file
                file.createNewFile();
            }
        } catch (IOException e) {
